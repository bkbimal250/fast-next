...
export default function AreaClient({
    ...
    const fetchLocationData = async () => {
        try {
            const parsed = await parseLocationSlugSmart(`${params.area}-${params.city}`);
            ...
            if (parsed.areaId) {
                ...
                setAreaName(parsed.area || params.area.replace(/-/g, ' '));
            } else {
    ...
    const faqSchema = {
        ...
        mainEntity: [
            {
                "@type": "Question",
                name: `Are there spa therapist jobs in ${areaName}, ${cityName}?`,
                acceptedAnswer: {
                    "@type": "Answer",
                    text: `Yes, we have multiple spa therapist openings in ${areaName}, ${cityName}. Browse this page to apply directly.`
                }
            },
            {
                "@type": "Question",
                name: `What is the salary for spa manager jobs in ${areaName}?`,
                acceptedAnswer: {
                    "@type": "Answer",
                    text: `Salaries for spa managers in ${areaName} vary by spa. Many offer competitive packages. View our listings for details.`
                }
            }
        ]
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex flex-wrap gap-4">
                    {[
                        ...
                        <Link
                            key={category.slug}
                            href={`/jobs/${category.slug}-jobs-in-${params.area}-${params.city}`}
                            className="bg-white border border-gray-200 rounded-lg px-6 py-3 hover:shadow-md transition-shadow"
                        >
                            <div className="font-semibold text-gray-900">{category.name} in {areaName}</div>
                            <div className="text-sm text-gray-600">View latest openings →</div>
                        </Link>
                    ))}
                </div>
            </div>
